...
- (void)socketDidDisconnect:(GCDAsyncSocket*)sock withError:(NSError*)err{
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        @Strongify(self)
        if (!Check_ValidString(self.IPStr)) return;
        if (!Check_ValidString(self.portStr)) return;
        [[TCPSocketClient sharedSocket] connectServerWithDelegate:self ToHost:self.IPStr onPort:[self.portStr intValue]];
    });
}
